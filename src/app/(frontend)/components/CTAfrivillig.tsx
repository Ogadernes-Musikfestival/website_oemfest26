    ...
    <Drawer.Root handle={frivilligDrawer} swipeDirection="right">
      <Drawer.Portal>
        ...
        <Drawer.Viewport className="fixed inset-0 flex items-center justify-end p-(--viewport-padding) [--viewport-padding:0px] supports-[-webkit-touch-callout:none]:[--viewport-padding:3.215rem]">
          <Drawer.Popup className="text-dark top-[2.5%] right-[2.5%] bottom-[2.5%] h-[95%] w-full max-w-7xl transform-[translateX(var(--drawer-swipe-movement-x))] touch-auto overflow-y-auto overscroll-contain rounded-3xl bg-white p-6 text-gray-900 outline outline-gray-200 transition-transform duration-450 ease-[cubic-bezier(0.32,0.72,0,1)] [--bleed:3rem] data-ending-style:transform-[translateX(calc(100%-var(--bleed)+var(--viewport-padding)))] data-ending-style:duration-[calc(var(--drawer-swipe-strength)*400ms)] data-starting-style:transform-[translateX(calc(100%-var(--bleed)+var(--viewport-padding)))] data-swiping:select-none supports-[-webkit-touch-callout:none]:mr-0 supports-[-webkit-touch-callout:none]:w-[20rem] supports-[-webkit-touch-callout:none]:max-w-[calc(100vw-20px)] supports-[-webkit-touch-callout:none]:rounded-[10px] supports-[-webkit-touch-callout:none]:pr-6 supports-[-webkit-touch-callout:none]:[--bleed:0px] sm:p-8 md:p-12 md:pr-18 dark:outline-gray-300">
            <Drawer.Content className="mx-auto h-full w-full">
              <div className="flex justify-between gap-x-6">
                <Drawer.Title className="mb-8 text-base font-medium md:-mt-1.5 md:text-lg">
